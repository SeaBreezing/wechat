...
class CrossAtt(nn.Module):
    ...
    def __init__(self, query_size, key_size, value_size, num_hiddens, num_heads, bias = False, **kwargs):
        ...
        self.bias = bias
        self.W_k = nn.Linear(key_size, num_hiddens, bias = bias) # (32, 330)
        self.W_q = nn.Linear(query_size, num_hiddens, bias = bias) # (330, 330)
    ...
    def forward(self, queries, keys, values, mask, dropout):
        ...
        output = self.DotProductAttention(queries, keys, values, valid_lens, dropout) # ([48, 330, 256])
        output_concat = transpose_output(output, self.num_heads) # ([16, 330, 768])
        return self.W_o(output_concat)

    def DotProductAttention(self, queries, keys, values, valid_lens, dropout):
        d = queries.shape[-1]
        scores = torch.bmm(queries, keys.transpose(1, 2)) / math.sqrt(d)
        self.attention_weights = self.masked_softmax(scores, valid_lens)
        drop_out = nn.Dropout(dropout)
        return torch.bmm(drop_out(self.attention_weights), values)
...
class MultiModal(nn.Module):
    def __init__(self, args):
        ...
        self.crossAttention_visual = CrossAtt(768, 768, 768, num_hiddens = 768, num_heads = 3)
        self.crossAttention_textual = CrossAtt(768, 768, 768, 768, 3)
    ...
    def forward(self, inputs, inference=False):
        ...
        # )
        # combine_random_attention_mask = combine_attention_mask[:, None, None, :] # ([16, 362]) -> ([16, 1, 1, 326])
        ...
        # )["last_hidden_state"] # sequence_output ([16, 362, 768])
        ...
        # vision_random_attention_mask = inputs["frame_mask"][:, None, None, :] # 
        # vision_random_attention_mask = (1.0 - text_random_attention_mask) * -10000.0
        ...
        #开始cross attention
        cro_visual_emb = self.crossAttention_visual(text_bert_output, vision_embedding, vision_embedding, inputs["title_mask"], dropout = 0.3)
        cro_textual_emb = self.crossAttention_textual(vision_embedding, text_bert_output, text_bert_output, inputs["frame_mask"], dropout = 0.3)
