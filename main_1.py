...
def train_and_validate(args):
    ...
    for i in range(K_foldNum):
        # 1. load data
        train_dataloader, val_dataloader = create_dataloaders(args, i) # 第i折
        ...
        num_total_steps = len(train_dataloader) * args.max_epochs # 28125
        fgm = FGM(model, eps=0.5)  # FGM
        ema = EMA(model, 0.9995)  # EMA
        ema.register()
        valid_f1 = 0
        f = open("training log.txt", "w")
        for epoch in range(args.max_epochs): # 5个epoch
            loop = tqdm(train_dataloader, total=len(train_dataloader)) # 返回的是可迭代对象，tqdm另外还有进度条的各种功能
            for batch in loop:
                model.train()
